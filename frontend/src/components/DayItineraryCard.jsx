...
const DayItineraryCard = ({ day, onDragEnd, fetchNearbyPlaces, t, onUpdateActivity, onDeleteActivity }) => {
  ...
  return (
    <motion.div
      ...
      className="mb-10 bg-white/90 dark:bg-gray-800/90 p-6 rounded-xl shadow-xl border border-gray-150 dark:border-gray-700/50" // Softer background, more diffused shadow, lighter border
      ...
      <DragDropContext onDragEnd={onDragEnd}>
        <Droppable droppableId={`activities-${day.day}`}>
          {(provided) => (
            ...
            >
              {/* Cinematic Vertical Timeline Line */}
              <div className="absolute left-[13px] top-6 bottom-6 w-[3px] bg-gradient-to-b from-blue-500 via-indigo-500 to-purple-500 rounded-full opacity-55 dark:opacity-35 pointer-events-none timeline-line" />
                ...
                >
                  {(provided, snapshot) => (
                    <ItineraryCard
                      ...
                      onUpdateActivity={(updatedAct) => onUpdateActivity(day.day, activityIndex, updatedAct)}
                      onDeleteActivity={() => onDeleteActivity(day.day, activityIndex)}
                    />
                  )}
                </Draggable>
              ))}
